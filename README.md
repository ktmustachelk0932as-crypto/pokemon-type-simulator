...
- **代替フォルム対応**: フォルムや性別によってタイプ・特性・種族値が異なるポケモンは、別の候補として検索可能
  ...
  - メガシンカ（メガリザードンX/Y等の従来分に加え、Pokémon Legends Z-Aで追加された新メガシンカも収録）
...
- **代替フォルム取得**: `ALT_FORM_MAP`に登録されたフォルム（リージョンフォーム・デオキシス・霊獣フォルム・性別違い・メガシンカ・ゲンシカイキ等）を種族ごとに抽出
...
2. **タイプを手動選択**
   - 「調べたいポケモンのタイプ」セクションから、最大2つまでタイプを選択
...
- **差分取得機能**: 既存の`pokemon.json`と比較し、新しいポケモンのみを取得
  ...
  - 既存データが最新の場合: 取得をスキップ
- **代替フォルム取得**: スクリプト内の`ALT_FORM_MAP`に登録されたフォルム（リージョンフォーム・ロトムの家電フォルム・デオキシス・霊獣フォルム・性別違い・メガシンカ・ゲンシカイキ等）を`pokemon-regional-forms.json`へ保存
